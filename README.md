...
> **Status:** v0.3.0. Works for IPv4 and IPv6 TCP/UDP traffic when the WinDivert driver files are present and the app is running as Administrator. See [Known limitations](#known-limitations).
...
* Use the dashboard to see total traffic, visible app/process counts, active rules, and engine health at a glance.
...
* Keyboard shortcuts for common actions: **F5** refreshes running apps and **Ctrl+N** creates a rule.
...
1. Find the program in the **Running applications** panel. Use search, or untick "Hide Microsoft" if you're looking for something system-level. Expand a group to pick a specific instance.
...
The **Settings** button in the top right opens a dedicated preferences view, where you can:
...
Imported profiles are validated before they are stored: unsupported schemas, invalid process IDs, negative limits, empty match values, and duplicate match targets are rejected.

---

## How it works (the short version)

The app sits between Windows and your network card, looking at IPv4 and IPv6 TCP/UDP packets going in or out. For each packet it figures out which program owns it, checks if you have a rule for that program, and if so passes the packet through a "token bucket" that paces it to your chosen speed limit. Packets that need pacing are copied into a bounded delayed-send queue and reinjected when due, so unrelated packets can keep moving through the capture loop. The live figures count packets that were successfully reinjected, so they reflect effective shaped traffic rather than queue intake. The remote side notices the slowdown and naturally throttles itself.
...
If the capture worker faults unexpectedly, BandwidthDesk closes its WinDivert handle and stops shaping so normal connectivity is restored instead of leaving a stalled capture session behind.
...
* **Best-effort PID matching.** The process-to-port lookup refreshes about every 250ms, so very short bursts may slip through before a rule catches them.
...
* **Bounded delay queue.** If the delayed-send queue is saturated, excess limited traffic is dropped rather than allowed to bypass the cap. TCP retransmission/backoff normally absorbs this; rate-limited UDP can lose datagrams under sustained overload.
